...
- **Location**: `.claude-memory-compiler/` at the host project's root (not nested under `.claude/`). The directory is git-tracked so every branch and worktree gets the compiler code.
- **Code vs data split**: `AGENTS.md`, `README.md`, `pyproject.toml`, `uv.lock`, `hooks/*.py`, `scripts/*.py`, and an inner `.gitignore` are tracked. Data is local-only and gitignored by the inner `.gitignore`: `.venv/`, `daily/`, `knowledge/`, `reports/`, `scripts/state.json`, `scripts/last-flush.json`, `scripts/flush.log`, `scripts/compile.log`, `scripts/session-flush-*`, `scripts/flush-context-*`.
- **Hook routing (why worktrees share one KB)**: The three hook commands in the host's `.claude/settings.json` begin with `cd "$(git rev-parse --path-format=absolute --git-common-dir)/.." && ...`. `git-common-dir` resolves to the main checkout's `.git` regardless of which worktree fires the hook, so every session writes to and reads from `<main-checkout>/.claude-memory-compiler/daily/` and `knowledge/`. Without this cd, each worktree would write to its own copy and fragment the KB.
